...
{
	ScriptableObject2D::ScriptableObject2D(const std::shared_ptr<Scene>& scene, std::unique_ptr<ScriptModule> script):
		GameObject2D(scene)
	{
		ADD_SCRIPT();
	}

	ScriptableObject2D::ScriptableObject2D(const std::shared_ptr<Scene>& scene, std::unique_ptr<ScriptModule> script, Components::TransformComponent2D transform):
		GameObject2D(scene, transform)
	{
		ADD_SCRIPT();
	}

	ScriptableObject3D::ScriptableObject3D(const std::shared_ptr<Scene>& scene, std::unique_ptr<ScriptModule> script):
		GameObject3D(scene)
	{
		ADD_SCRIPT();
	}

	ScriptableObject3D::ScriptableObject3D(std::shared_ptr<Scene> scene, std::unique_ptr<ScriptModule> script, Components::TransformComponent3D transform):
	...
	ScriptableSpriteObject2D::ScriptableSpriteObject2D(std::shared_ptr<Scene> scene, std::unique_ptr<ScriptModule> script, SpriteType type):
